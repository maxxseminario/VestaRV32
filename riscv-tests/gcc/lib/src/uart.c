...
void uartx_puts(UARTx_t* UARTx, const void* str)
{
	...
	// Arguments:
	//	UARTx: a pointer to the desired UART peripheral base address (e.g. UART0, UART1, etc.)
	//	str: A pointer to the string to send. Must be properly null terminated. Note: although the type of this string is const void*, you should pass it a string (char*) type. This was done to avoid some irritating compiler issues.
	// Returns: nothing
...
void uartx_getbytes(UARTx_t* UARTx, void* buf, uint32_t len)
{
	...
	// Arguments:
	//	UARTx: a pointer to the desired UART peripheral base address (e.g. UART0, UART1, etc.)
	//	buf: The array to store the received bytes into. Size must be at least len. Note: although the type of this string is void*, you should pass it a whatever array type you want (uint8_t*, char*, uint32_t*, etc.) type. This was done to avoid some irritating compiler issues.
