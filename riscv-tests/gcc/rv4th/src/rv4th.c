...
#define USER_PROG_SIZE 512	// * 2 bytes

//max number of user-defined words
#define USER_OPCODE_MAPPING_SIZE 64	// * 2 bytes

//total string length of all word names (+ 1x<space> each)
#define USER_CMD_LIST_SIZE 256	// * 1 byte

//maximum input line length
#define LINE_BUFFER_SIZE 256	// * 1 byte
...
const char cmdListBi[] = {
	...
	"fr fw fe sbi cbi "					// 81 -> 85
	"mask and or swphw sll "			// 86 -> 90
	"srl mr mw me wadc "				// 91 -> 95
};
...
static void flashWritePageFunc(uint32_t page_address, int32_t mode);
...
void flashWritePageFunc(uint32_t page_address, int32_t mode)
{
	...
	// Mode indicates the mode data type of the payload
	// If mode == 0: Expects the payload to be a HEX string with exactly 512 characters that represent a 256-byte page. Each group of two characters will be treated as one MSB-first byte in HEX. The bytes must be ordered such that the first byte will be stored at the location "page_address", the second at "page_address + 1", and so on.
	// If mode == 1: Expects the payload to be a raw binary array containing exactly 256 bytes to form a page. Each byte must be MSB-first. The bytes must be ordered such that the first byte will be stored at the location "page_address", the second at "page_address + 1", and so on.
	// If mode > 1: Expects a payload compressed using LZW compression. The uncompressed version of the payload must be identical to the specification for if mode were equal to 1. Furthermore, mode must be equal to the number of compressed bytes in the (compressed) payload.
	...
	uint8_t flash_write_buffer[257];
	uint16_t i, payload_length;
	if (mode > 1)
	{
		payload_length = mode;
		if (payload_length > 257)
		{
			goto bad_flashWritePageFunc;
		}
	}
	else
		payload_length = 256;
	...
	// If the data was compressed, decompress it
	if (mode > 1)
	{
		uint8_t buf[256];
		if (lzw_decompress(flash_write_buffer, payload_length, buf, 256) != 256)
		{
			goto bad_flashWritePageFunc;
		}
...
void memoryReadFunc(uint32_t start_address, uint32_t length, int32_t mode)
{
	...
	//	mode: If mode == 0, sends an ASCII hex string of the data. If mode == 1, sends a raw binary payload of the data. If mode == 2, sends an LZW compressed version of the data, beginning with two bytes indicating the length of the compressed payload (the first is the LSbyte and the second is the MSbyte), followed by the compressed payload.

	// Compute the CRC16_CDMA2000 value of the data (polynomial = 0xC857, initial CRC value = 0xFFFF, no final XOR (= 0x0000), no data reflection, no output reflection)
	...
	// Compress the data (if in compression mode)
	if (mode == 2)
	{
		if (length > 65535)
			goto bad_memoryReadFunc;
		uint8_t compressed[length];
		int compressed_length = lzw_compress(data, length, compressed, length);
		if (compressed_length <= 0)
			goto bad_memoryReadFunc;
		data = compressed;
		length = compressed_length;

		// Print the length in two bytes, LSbyte first
		uart_putchar(compressed_length);		// LSbyte
		uart_putchar(compressed_length >> 8);	// MSbyte
	}
...
void memoryEraseFunc(uint32_t start_address, uint32_t length)
{
	// Sets all 4-byte words to 0x00000000 beginning at start_address and continuing for length bytes
	// Arguments:
	//	start_address: The first address. Must be divisible by 4
	//	length:	The number of bytes to erase. Must be divisible by 4
	start_address &= 0xFFFFFFFC;
	uint32_t end_address = start_address + (length & 0xFFFFFFFC);
...
void waveformAdcTestFunc(uint32_t totalCounts)
{
	// You must set up the bias generator and ADC registers before running this function!

	// Progress bar
	uint32_t pct = 0;
	uint32_t pctCountsIncrement = totalCounts / 10;
	uint32_t currentPctThreshold = totalCounts - (totalCounts % 10) - pctCountsIncrement;
	static uint32_t wadcRunCount = 1;
	printString("Starting wadc #");
	uart_putui(wadcRunCount);
	printStringln("...");
	printString("0 % ");

	// Enable the ADC and disable the DMA, the auto trigger, the adjusted data alignment, and the interrupts
	ADC0SR = 0;
	ADC0CR &= ~(ADCDEN | ADCTS_MASK | ADCDAL | ADCBFIE | ADCCIE);
	ADC0CR |= ADCEN;
	...
	// Do totalCounts ADC conversions
	while (totalCounts > 0)
	{
		// Calculate percent while waiting for ADC
		if (totalCounts < currentPctThreshold)
		{
			pct += 10;
			currentPctThreshold -= pctCountsIncrement;
			printNumberUInt32(pct);
			printString("% ");
		}

		// Wait for the ADC to finish converting and the data to be available
		while (ADC0SR & ADCACT) {}

		// Start the next ADC conversion
		ADC0CR |= ADCSC;

		// Increment the bin corresponding to the ADC output value
		hist[ADC0VAL]++;
	...
	printStringln("100 %");
	printString("wadc #");
	printNumberUInt32(wadcRunCount);
	printStringln("done. Don't forget to switch off the ATPx switches!");
	wadcRunCount++;

	// Print out the results
	printStringln("ADC DNL histogram:");
	for (i = 0; i < 1024; i++)
	{
		uart_putui(hist[i]);
		uart_putchar(',');
...
GCC_DIAG_ON(int-to-pointer-cast);
		...
		case 82: // fw  ( mode page_address -- ) \ SPI flash write page
			i = popMathStack();	// page_address, the address of the page to write to
			j = popMathStack();	// mode, if true (nonzero), expects the transmission of binary data, otherwise it expects an ASCII hex string
		...
		case 84: // sbi ( bitnum addr -- ) \ set bit by writing directly to memory address (words only!)
			i = popMathStack();	// address to write to
			i = i >> 2;
		...
		case 85: // cbi ( bitnum addr -- ) \ clear bit by writing directly to memory address (words only!)
			i = popMathStack();	// address to write to
			i = i >> 2;
		...
		case 86: // mask ( posbitmask val addr -- ) \ write only masked bits by writing directly to memory address (words only!)
			i = popMathStack();	// address to write to
			i = i >> 2;
		...
		case 87: // and ( a b -- a&&b ) \ Logical and (if a and b are nonzero, return 1)
			i = popMathStack();	// b
			TOS = TOS && i;
			break;
		
		case 88: // or ( a b -- a||b ) \ Logical or (if a or b are nonzero, return 1)
			i = popMathStack();	// b
			TOS = TOS || i;
			break;
		
		case 89: // swphw  ( n -- n ) \ Swap upper and lower 16 bits of TOS
			TOS = ((TOS >> 16) & 0x0000ffff) | ((TOS << 16) & 0xffff0000);
			break;
		
		case 90: // sll  ( val bits -- ret ) \ Shift left logical (also arithmetic)
			i = popMathStack();	// bits
			TOS = ((uint32_t)TOS) << i;
			break;
		
		case 91: // srl  ( val bits -- ret ) \ Shift right logical
			i = popMathStack();	// bits
			TOS = ((uint32_t)TOS) >> i;
			break;
		
		case 92: // mr  ( mode length start_address -- ) \ Memory read
			i = popMathStack();	// start_address, the address to begin reading from
			j = popMathStack();	// length, the number of bytes to read
		...
		case 94: // me ( length start_address -- ) \ Memory erase
			i = popMathStack();	// start_address, the address to begin erasing from
			j = popMathStack();	// length, the number of bytes to erase
			memoryEraseFunc(i, j);
			break;
		
		case 95:	// wadc ( totalCounts -- ) \ Waveform ADC test (for DNL)
			i = popMathStack();
			waveformAdcTestFunc(i);
			break;
...
void rv4th_init()
{
	...
	lineBufferIdx = 0;
	printStringln((char *)"rv4th-flash!");
