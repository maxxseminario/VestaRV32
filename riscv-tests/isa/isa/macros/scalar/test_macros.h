...
#define GPIO_OUTPUT_REG  0x04
#define GPIO_DIR_REG     0x14
#define GPIO_INPUT_REG   0x00
#define GPIO_PULLUP_REG  0x28

/* Test Result Macros */
#define GPIO_TEST_PASS   0xFFFFFFFF
#define GPIO_TEST_FAIL   0x00000000
 ...
 */
#define TEST_GPIO_OUTPUT(testnum, gpio_base, pin_mask, value) \
    TEST_CASE(testnum, x14, GPIO_TEST_PASS, \
        li  x1, gpio_base;                /* Load GPIO base address */ \
        li  x2, pin_mask;                 /* Pin mask */ \
        sw  x2, GPIO_DIR_REG(x1);         /* Set direction to output */ \
        li  x3, value;                    /* Value to write */ \
        sw  x3, GPIO_OUTPUT_REG(x1);      /* Write to output register */ \
        lw  x4, GPIO_OUTPUT_REG(x1);      /* Read back */ \
        li  x14, GPIO_TEST_PASS;          /* Default to pass */ \
        bne x3, x4, 1f;                   /* Compare written/read */ \
        j   2f;                           /* Skip fail case */ \
    1:  li  x14, GPIO_TEST_FAIL;          /* Mark as failed */ \
    2:  nop;                              /* Continue */ \
    )
