...
/** Defines **/
#define LZW_MAX_BITS		11
#if (LZW_MAX_BITS < 9 || LZW_MAX_BITS > 12)
#error "LZW_MAX_BITS must be within the range [9, 12]"
#endif
#define LZW_TOTAL_CODES     (1 << LZW_MAX_BITS)
...
#define WRITE_CODE(code,maxcode) do {                           \
	...
	do { if (dst_index >= dst_max_bytes) { return -1; } dst[dst_index++] = shifter; shifter >>= 8; output_bytes++;        \
	} while ((bits -= 8) >= 8);                                 \
} while (0)
...
{
	// Compresses LZW-compressed data from src byte array to dst byte array
	// Returns negative numbers for errors, positive numbers for success. When successful, the return value is the number of bytes in the compressed array
	// The first byte in the dst array indicates the maximum number of bits used in the dictionary. In this implementation, this is represented with a preprocessor define LZW_MAX_BITS, and is not configurable at runtime.
	int next = FIRST_STRING, prefix = NULL_CODE, bits = 0, c;
	unsigned long input_bytes = 0, output_bytes = 0;
	unsigned long shifter = 0;
	unsigned int src_index = 0, dst_index = 1;


	// Based on the "maxbits" parameter, compute total codes and allocate dictionary storage
   short first_references[LZW_TOTAL_CODES];
   short next_references[LZW_TOTAL_CODES - 256];
   unsigned char terminators[LZW_TOTAL_CODES - 256];

	// Clear the dictionary
	int i;
	for (i = 0; i < LZW_TOTAL_CODES; i++)
	{
		first_references[i] = 0;
	}
	for (i = 0; i < (LZW_TOTAL_CODES - 256); i++)
	{
		next_references[i] = 0;
		terminators[i] = 0;
	}
	...
	if (0 >= dst_max_bytes)
		return -1;
	dst[0] = LZW_MAX_BITS;	// first byte in output stream indicates the maximum symbol bits

	// This is the main loop where we read input bytes and compress them. We always keep track of the "prefix", which represents a pending byte (if < 256) or string entry (if >= FIRST_STRING) that has not been sent to the decoder yet. The output symbols are kept in the "shifter" and "bits" variables and are sent to the output every time 8 bits are available (done in the macro).
	while (src_index < src_bytes) {
		c = src[src_index];
		src_index++;
		...
		if (!cti) {
			...
			if (++next == LZW_TOTAL_CODES || output_bytes > 8 + input_bytes + (input_bytes >> 4)) {
				if (next < LZW_TOTAL_CODES)
	...
	// Done with input, so if we've received anything we still need to send that pesky pending prefix...
	if (prefix != NULL_CODE) {
		WRITE_CODE (prefix, next);

		if (++next == LZW_TOTAL_CODES)  // watch for clearing to the first string to stay in step with the decoder! (this was actually a corner-case bug that did not trigger often)
			next = FIRST_STRING;
	}
...
{
	// Decompresses LZW-compressed data from src byte array to dst byte array
	// Returns negative numbers for errors, positive numbers for success. When successful, the return value is the number of bytes in the decompressed array
	// The first byte in the src array indicates the maximum number of bits used in the dictionary. In this implementation, this is represented with a preprocessor define LZW_MAX_BITS, and is not configurable at runtime. If LZW_MAX_BITS does not agree with the first byte of the src array, the decompressor will fail and return an error.
	int read_byte, next = FIRST_STRING, prefix = CLEAR_CODE, bits = 0;
	unsigned long shifter = 0;
	unsigned int src_index = 1, dst_index = 0;

	read_byte = src[0];
	// Based on the "maxbits" parameter, compute total codes and allocate dictionary storage
	if (read_byte != LZW_MAX_BITS)
		return -2;

	unsigned char reverse_buffer[LZW_TOTAL_CODES - 256];
	short prefixes[LZW_TOTAL_CODES - 256];
	unsigned char terminators[LZW_TOTAL_CODES - 256];

	// This is the main loop where we read input symbols. The values range from 0 to the code value of the "next" string in the dictionary (although the actual "next" code cannot be used yet, and so we reserve that code for the END_CODE). Note that receiving an -1 from the input stream is actually an error because we should have gotten the END_CODE first.
	...
	while (1) {
		...
		if (code == next)                   // sending the maximum code is reserved for the end of the file
			...
			if (dst_index >= dst_max_bytes)
				return -1;
			dst[dst_index] = code;	// literally and becomes our initial prefix
			dst_index++;
			next++;
		}
		// Otherwise we have a valid prefix so we step through the string from end to beginning storing the bytes in the "reverse_buffer", and then we send them out in the proper order. One corner-case we have to handle here is that the string might be the same one that is actually being defined now (code == next-1). Also, the first 256 entries of "terminators" and "prefixes" are fixed and not allocated, so that messes things up a bit.
			...
			c = *--rbp;     // the first byte in this string is the terminator for the last string, which is the one that we'll create a new dictionary entry for this time

			do { if (dst_index >= dst_max_bytes) { return -1; } dst[dst_index++] = *rbp; }	// send string in corrected order (except for the terminator which we don't know yet)
			...
			if (code == next - 1)
			{
				if (dst_index >= dst_max_bytes)
					return -1;
				dst[dst_index] = c;	// literally and becomes our initial prefix
				dst_index++;
			}
			...
			if (++next == LZW_TOTAL_CODES)              // check for full dictionary, which forces a reset (and, BTW,
